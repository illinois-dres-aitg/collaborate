...
#                               label=_(u'username'))
    username = forms.RegexField(label=_("Username"), max_length=30, regex=r'^\w+$', 
                                help_text = _("30 characters or fewer. Alphanumeric characters only (letters, digits and underscores)."),
...
#            raise forms.ValidationError(_(u'Usernames can only contain letters, numbers and underscores'))
    ...
    def clean_email(self):
        """
        Validates that the email address is of the right format and not already
        in use.
        """
        users = User.objects.filter(email__exact=self.cleaned_data['email'])
        if not users:
...
class ResendKeyForm(forms.Form):
    """Class that requests an email address. It is used to 
    resend an activation key."""
