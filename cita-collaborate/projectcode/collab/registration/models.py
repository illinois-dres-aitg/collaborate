    ...
    def activate_user(self, activation_key):
        ...
        if SHA1_RE.search(activation_key):
            ...
            if not profile.activation_key_expired():
                ...
                new_activation_key = sha.new(salt+user.username).hexdigest()
#                profile.activation_key = new_activation_key
                profile.activation_key = "AlreadyActivated" + new_activation_key[:-16]
                profile.save()
                return user
        return False
        ...
        if send_email:
            ...
            message = render_to_string('registration/activation_email.txt',
                                       { 'activation_key': registration_profile.activation_key,
                                         ...
                                         'site': current_site })
            send_mail(subject, message, settings.REGISTRATION_FROM_EMAIL, [new_user.email])
        return new_user
